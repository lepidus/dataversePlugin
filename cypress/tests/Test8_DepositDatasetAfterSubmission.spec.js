...
describe('Test research data deposit in dataset tab', function() {
    ...
    it('Research data cannot be deposited without files', function() {
        ...
        cy.get('.app__notifications').contains('Research data is required. Please ensure that you have chosen and uploaded research data.');
    });
    ...
    it('Research data deposited in dataset tab', function() {
		...
		cy.get('#datasetData > .value > p').contains(
			'Corino, Carlo, ' + currentYear + ', "The influence of lactation on the quantity and quality of cashmere production"'
		);
		cy.get('#datasetData .value > p > a').contains(/https:\/\/doi\.org\/10\.[^\/]*\/FK2\//);
		cy.get('#datasetData .value > p').contains(
